...
- ```@ValidateDto```: add Jakarta Bean Validation capabilities
- ```@NestedMapping```: Add support for custom nested classes (DTOs use other DTOs, not the custom classes themselves. If 
Bird.java has a field "private Voice voice", the corresponding BirdDto.java should usually have a field 
```private VoiceDto voice``` rather than ```private Voice voice``` like in the base class).
- Refactorings according to SonarQube warnings
