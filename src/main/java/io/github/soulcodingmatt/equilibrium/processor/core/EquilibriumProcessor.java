...
public class EquilibriumProcessor extends AbstractProcessor {
  ...
  public static final String INVALID_FIELD_NAME_IN_IGNORE_LIST =
      "Invalid field name in ignore list: '";
  ...
  public synchronized void init(ProcessingEnvironment processingEnv) {
    ...
    try {
      ...
      trees = null; // Trees may be unavailable in some environments
    }
  }
  ...
  private boolean validateUniqueDtoCombinations(
      TypeElement classElement, GenerateDto[] annotations) {
    ...
    for (GenerateDto annotation : annotations) {
      ...
      if (!uniqueCombinations.add(combination)) {
        error(
            classElement,
            "Duplicate DTO configuration would generate the same class: " + combination);
        return false;
      }
      ...
      if (id != -1 && !usedIds.add(id)) {
        error(
            classElement,
            "Duplicate DTO ID: " + id + ". Each @GenerateDto annotation must have a unique ID.");
        return false;
      }
    }
  ...
  private void processGenerateDto(TypeElement classElement, GenerateDto annotation) {
    try {
      ...
      error(
          classElement,
          "Failed to generate DTO: " + e.getMessage() + " (" + e.getClass().getSimpleName() + ")");
    }
  }
                ...
                for (int id : ids) {
                  if (!uniqueIds.add(id)) {
                    messager.printMessage(
                        ...
                        DUPLICATE_ID
                            + id
                            + " in @IgnoreDto annotation for field '"
                            + field.getSimpleName()
                            + DUPLICATES_WILL_BE_IGNORED,
                        field);
                  } else if (!validDtoIds.isEmpty() && !validDtoIds.contains(id)) {
                    // Only check for orphaned IDs if there are actually IDs defined in @GenerateDto
                    // annotations
                    messager.printMessage(
                        Diagnostic.Kind.WARNING,
                        "ID "
                            ...
                            + " in @IgnoreDto annotation for field '"
                            + field.getSimpleName()
                            + "' does not correspond to any @GenerateDto annotation ID",
                        field);
                  }
                }
              }
  ...
  private boolean validateUniqueRecordCombinations(
    ...
    for (GenerateRecord annotation : annotations) {
      ...
      if (!uniqueCombinations.add(combination)) {
        error(
            classElement,
            "Duplicate Record configuration would generate the same class: " + combination);
        return false;
      }
      ...
      if (id != -1 && !usedIds.add(id)) {
        error(
            ...
            "Duplicate Record ID: "
                + id
                + ". Each @GenerateRecord annotation must have a unique ID.");
        return false;
      }
    }
  ...
  private void processGenerateRecord(TypeElement classElement, GenerateRecord annotation) {
    try {
      ...
      RecordGenerator generator =
          new RecordGenerator(classElement, packageName, className, ignoredFields, recordId, filer);
      generator.generate();
  ...
  private void validateIgnoreRecordAnnotations(
      TypeElement classElement, Set<Integer> validRecordIds) {
                ...
                for (int id : ids) {
                  if (!uniqueIds.add(id)) {
                    messager.printMessage(
                        ...
                        DUPLICATE_ID
                            + id
                            + " in @IgnoreRecord annotation for field '"
                            + field.getSimpleName()
                            + DUPLICATES_WILL_BE_IGNORED,
                        field);
                  } else if (!validRecordIds.isEmpty() && !validRecordIds.contains(id)) {
                    // Only check for orphaned IDs if there are actually IDs defined in
                    // @GenerateRecord annotations
                    messager.printMessage(
                        Diagnostic.Kind.WARNING,
                        "ID "
                            + id
                            + " in @IgnoreRecord annotation for field '"
                            + field.getSimpleName()
                            + "' does not correspond to any @GenerateRecord annotation ID",
                        field);
                  }
                }
              }
  ...
  private boolean validateUniqueVoCombinations(TypeElement classElement, GenerateVo[] annotations) {
    ...
    for (GenerateVo annotation : annotations) {
      ...
      if (!uniqueCombinations.add(combination)) {
        error(
            classElement,
            "Duplicate VO configuration would generate the same class: " + combination);
        return false;
      }
      ...
      if (id != -1 && !usedIds.add(id)) {
        error(
            classElement,
            "Duplicate VO ID: " + id + ". Each @GenerateVo annotation must have a unique ID.");
        return false;
      }
    }
  ...
  private void processGenerateVo(TypeElement classElement, GenerateVo annotation) {
    try {
      ...
      VoGenerator generator =
          new VoGenerator(
              classElement, packageName, className, ignoredFields, generateSetter, voId, filer);
      generator.generate();
                ...
                for (int id : ids) {
                  if (!uniqueIds.add(id)) {
                    messager.printMessage(
                        ...
                        DUPLICATE_ID
                            + id
                            + " in @IgnoreVo annotation for field '"
                            + field.getSimpleName()
                            + DUPLICATES_WILL_BE_IGNORED,
                        field);
                  } else if (!validVoIds.isEmpty() && !validVoIds.contains(id)) {
                    // Only check for orphaned IDs if there are actually IDs defined in @GenerateVo
                    // annotations
                    messager.printMessage(
                        Diagnostic.Kind.WARNING,
                        "ID "
                            ...
                            + " in @IgnoreVo annotation for field '"
                            + field.getSimpleName()
                            + "' does not correspond to any @GenerateVo annotation ID",
                        field);
                  }
                }
              }
  ...
  /**
   * Validates all ValidateRecord annotations in a class for type compatibility and logical
   * conflicts.
   *
   * @param classElement the class to validate
   * @return true if validation passes, false if there are errors
   */
  private boolean validateValidateRecordAnnotations(TypeElement classElement) {
    boolean hasErrors = false;

    // Check all fields for ValidateRecord annotations
    for (Element element : classElement.getEnclosedElements()) {
      if (element.getKind() == ElementKind.FIELD) {
        VariableElement field = (VariableElement) element;

        // Get all ValidateRecord annotations on this field (handles both single and repeatable)
        ValidateRecord[] validateRecordAnnotations =
            field.getAnnotationsByType(ValidateRecord.class);

        if (validateRecordAnnotations.length > 0) {
          // Validate the field
          List<String> errors =
              ValidationConflictUtil.validateRecordField(field, validateRecordAnnotations);
  ...
  /**
   * Validates all ValidateVo annotations in a class for type compatibility and logical conflicts.
   *
   * @param classElement the class to validate
   * @return true if validation passes, false if there are errors
   */
  private boolean validateValidateVoAnnotations(TypeElement classElement) {
    boolean hasErrors = false;

    // Check all fields for ValidateVo annotations
    for (Element element : classElement.getEnclosedElements()) {
      if (element.getKind() == ElementKind.FIELD) {
        VariableElement field = (VariableElement) element;

        // Get all ValidateVo annotations on this field (handles both single and repeatable)
        ValidateVo[] validateVoAnnotations = field.getAnnotationsByType(ValidateVo.class);

        if (validateVoAnnotations.length > 0) {
          // Validate the field
          List<String> errors =
              ValidationConflictUtil.validateVoField(field, validateVoAnnotations);
