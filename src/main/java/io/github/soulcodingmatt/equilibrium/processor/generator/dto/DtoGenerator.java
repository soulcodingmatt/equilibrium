...
public class DtoGenerator {
  ...
  public void generate() throws IOException {
    ...
    JavaFileObject sourceFile =
        filer.createSourceFile(packageName + "." + dtoClassName, classElement);
  ...
  private void writeImports(Writer writer, List<VariableElement> fields) throws IOException {
    ...
    NestedMappingResolver nestedResolver =
        new NestedMappingResolver(classElement, messager, dtoClassName);
  ...
  /**
   * Transforms the field type based on @NestedMapping annotations. Returns simple names for use in
   * field declarations, getters, setters, constructor.
   */
  private String getTransformedFieldType(VariableElement field) {
    NestedMappingResolver resolver =
        new NestedMappingResolver(classElement, messager, dtoClassName);
    return resolver.getTransformedFieldType(field);
  }

  /** Writes @Builder.Default annotation if applicable. */
  private void writeBuilderDefaultAnnotation(Writer writer, VariableElement field)
      throws IOException {
    if (!builder) {
    ...
    if (builderDefault != null) {
      ...
      // Emit @Builder.Default only if we'll have an initializer (explicit or inherited when
      // allowed)
      boolean willInherit =
  ...
  private String getTypeSpecificValue(VariableElement field, DtoBuilderDefault builderDefault) {
    ...
    if (isPrimitiveType(fieldType) || isWrapperType(fieldType)) {
      ...
      // Check if booleanValue is explicitly set (we can't distinguish default false from explicit
      // false)
  ...
  /**
   * Checks if any parameter other than booleanValue is set in the annotation. This helps determine
   * if booleanValue was explicitly set.
   */
  ...
  private boolean isPrimitiveType(String fieldType) {
    return Arrays.asList(
            INTEGER_STRING,
            LONG_STRING,
            TypeNames.SHORT_STRING,
            TypeNames.BYTE_STRING,
            TypeNames.FLOAT_STRING,
            TypeNames.DOUBLE_STRING,
            TypeNames.BOOLEAN_STRING,
            "char")
        .contains(fieldType);
  }
  ...
  /**
   * Gets the existing default value from a field that already has @Builder.Default. Uses a
   * pre-scanned map populated from the AST when possible.
   */
  ...
  private void writeField(
      Writer writer, CodeWriter code, DtoClassWriter classWriter, VariableElement field)
      throws IOException {
    ...
    // fieldDeclaration already includes "type name" and possibly "= default"; split to type/name
    // for emitter
