 ...
 *
 * <p>All patterns (package, postfix, and field names): - No spaces allowed - No special characters
 * allowed (except dots, underscores, and hyphens in package names) - No consecutive dots in package
 * names - No leading/trailing dots in package names - No null or empty values allowed
 *
 * <p>Package names: - Must start with a letter - Can contain letters, numbers, dots, underscores,
 * and hyphens - Parts must be separated by dots - Each part must start with a letter
 *
 * <p>Postfix and field names: - Can contain letters and numbers - No special characters allowed
 *
 * <p>Note: While this utility only enforces technical correctness, the default values and generated
 * names will follow Java naming conventions: - Package names: lowercase with dots - Class names
 * (postfix): PascalCase - Field names: camelCase
 */
public class ValidationUtil {
    ...
    for (String part : parts) {
      if (!PACKAGE_PART_PATTERN.matcher(part).matches()) {
        return PACKAGE_NAME
            ...
            + "' has invalid part '"
            + part
            + "'. Each part must start with a letter and contain only letters, numbers, underscores, and hyphens";
      }

      if (JAVA_KEYWORDS.contains(part.toLowerCase())) {
        return PACKAGE_NAME
            + pkg
            + "' contains Java reserved keyword '"
            + part
            + "'. Reserved keywords cannot be used in package names as they prevent class creation";
      }
    }
  ...
  public static boolean isValidFieldName(String fieldName) {
    return fieldName != null
        && !fieldName.isEmpty()
        && ALPHANUMERIC_PATTERN.matcher(fieldName).matches();
  }

  /**
   * Gets the default package name based on project coordinates and class type. This method follows
   * Java naming conventions for the generated package name.
   *
  ...
  /**
   * Gets the default postfix for a class type. This method follows Java naming conventions for the
   * generated postfix.
   *
   * @param classType the type of class being generated ("DTO", "Record", or "VO")
